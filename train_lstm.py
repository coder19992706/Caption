...
model=Sequential()
model.add(LSTM(32, input_shape=(1,29572),return_sequences=True))
model.add(LSTM(32))
...
#model.load_weights('my_model_weights_lstm.h5')
train_generator=data()
model.fit_generator(generator=train_generator,steps_per_epoch=len(features_dict),epochs=1)
model.save_weights('my_model_weights_lstm.h5')

ep=open('epochs_lstm.txt','w')
ep.write(str(epo+1))
ep.close()